...
{
  if (std::ranges::any_of(kumpulanTipeToken,
          [this](auto type)
          {
            return this->cekTipeToken(type);
          }) // function any_of
  )
  {
    return true;
...
const nusantara::Token& nusantara::PenguraiSintaks::makanToken(const nusantara::TipeToken& tipe, const std::string& pesanKesalahan)
{
  if (this->cekTipeToken(tipe))
  {
    return this->majuKeTokenSelanjutnya();
  }
  throw DataPengecualianSintaks{.lokasiBerkas = this->tokenSaatIni().lokasiBerkas, .lokasiToken = this->tokenSaatIni().lokasi, .konten = this->tokenSaatIni().konten, .pesan = pesanKesalahan};
} // function makanToken
...
void nusantara::PenguraiSintaks::uraikan()
{
  ...
  while (!this->apakahSudahDiAkhirFile())
  {
    try
    {
      auto titikPernyataan(this->uraiPernyataan());
      if (!titikPernyataan->ambilKumpulanTitikTurunan().empty())
      {
        titik.tambahTitikTurunan(std::move(titikPernyataan));
      }
    }
  ...
  try
  {
    if (this->apakahSudahDiAkhirFile())
    {
      if (!this->psa)
      {
        titik.tambahTitikTurunan(std::make_unique<Titik>(Titik{TipeTitik::TOKEN, this->tokenSaatIni()} // constructor Titik
    ...
    {
      throw DataPengecualianSintaks{.lokasiBerkas = this->tokenSaatIni().lokasiBerkas, .lokasiToken = this->tokenSaatIni().lokasi, .konten = this->tokenSaatIni().konten, .pesan = __NK__LABEL_KELUARAN_PS__ "harus nya akhir dari file."}; // throw
    } // else
  }
  catch (const DataPengecualianSintaks& data)
  {
...
std::unique_ptr<nusantara::Titik> nusantara::PenguraiSintaks::uraiPernyataan()
{
  ...
  {
    return this->makanToken(TipeToken::titikKoma,
        __NK__LABEL_KELUARAN_PS__ "Jangan lupa titik koma."); // fucntion makanToken
  ...
  auto makanTokenTitikKomaMendukungPsa = [&makanTokenTitikKoma, this, &titik] -> void
  {
    ...
    {
      titik.tambahTitikTurunan(std::make_unique<Titik>(Titik{TipeTitik::TOKEN, makanTokenTitikKoma()} // constructor Titik
          )                                                                                           // constructor make_unique
      );                                                                                              // function tambahTitikTurunan
    } // else
  };
  ...
  {
    Token tokenMuat = this->makanToken(TipeToken::muat,
        "Untuk memuat berkas, harus menggunakan kata kunci 'muat'."); // function makanToken

    auto ekspresi(this->uraiEkspresi());
#ifdef __NK__SINTAKS_PERNYATAAN_DENGAN_TITIK_KOMA_WAJIB__
    makanTokenTitikKoma();
#endif
    // AREA PARSER MEMUAT BERKAS
    const auto& tokenEkspresi = ekspresi->ambilKumpulanTitikTurunan()[0]->ambilToken().value();
    const auto lokasiBerkas = hapusKanKutipDiAwalDanAkhirString(tokenEkspresi.konten);

    bool sudahDiMuat = false;
    for (const auto& lokasiBerkasYgSudahDiMuat : *this->kumpulanlokasiBerkasYgSudahDiMuat)
    {
      sudahDiMuat = (lokasiBerkasYgSudahDiMuat == lokasiBerkas);
    }
    ...
    if (!sudahDiMuat)
    {
      try
      {
        titik = Titik{TipeTitik::TOKEN, tokenMuat}; // constructor Titik
        PemecahSintaks pemecahSintaks(nusantaraTokenRegexs());
        pemecahSintaks.aturLokasiBerkas(lokasiBerkas);
        pemecahSintaks.aturKonten(bacaBerkasDalamBentukString(lokasiBerkas));
        pemecahSintaks.tokenisasi();

        this->kumpulanlokasiBerkasYgSudahDiMuat->push_back(lokasiBerkas);
        PenguraiSintaks penguraiSintaks(this->psa, this->kumpulanlokasiBerkasYgSudahDiMuat);
        penguraiSintaks.aturKumpulanToken(pemecahSintaks.ambilHasil());
        penguraiSintaks.uraikan();

        penguraiSintaks.hasilPenguraian->hapusAkhirDariBerkasJikaAda();
        penguraiSintaks.hasilPenguraian->pindahKanSemuaTitikTurunanKe(titik);
      }
      catch (const std::exception& kesalahan)
      {
        throw DataPengecualianSintaks(tokenEkspresi.lokasiBerkas, tokenEkspresi.lokasi, tokenEkspresi.konten, __NK__LABEL_KELUARAN_PS__ + std::string(kesalahan.what()));
      }
    }
  }
  else
  {
    throw DataPengecualianSintaks{.lokasiBerkas = this->tokenSaatIni().lokasiBerkas,
...
std::unique_ptr<nusantara::Titik> nusantara::PenguraiSintaks::uraiPanggilFungsi()
{
  ...
  titik.tambahTitikTurunan(std::make_unique<Titik>(Titik{
      TipeTitik::TOKEN,
      this->makanToken(nusantara::TipeToken::identifikasi,
          __NK__LABEL_KELUARAN_PS__ "Nama fungsi belum dibuat.") // function makanToken
  } // constructor Titik
      ) // constructor make_unique
  );    // function tambahTitikTurunan
  ...
  {
    return this->makanToken(TipeToken::kurungBulatBuka,
        __NK__LABEL_KELUARAN_PS__ "Tempat parameter pada fungsi harus ada kurung bulat buka '('."); // function makanToken
  }; // lambda makanTokenKurungBulatBuka

  auto makanTokenKurungBulatTutup = [this] -> const nusantara::Token&
  {
    return this->makanToken(TipeToken::kurungBulatTutup,
        __NK__LABEL_KELUARAN_PS__ "Tempat parameter pada fungsi harus di akhiri dengan kurung tutup ')'."); // function makanToken
  ...
  {
    titik.tambahTitikTurunan(std::make_unique<Titik>(Titik{TipeTitik::TOKEN, makanTokenKurungBulatBuka()} // constructor Titik
        )                                                                                                 // constructor make_unique
    );                                                                                                    // function tambahTitikTurunan

    titik.tambahTitikTurunan(std::make_unique<Titik>(Titik{TipeTitik::TOKEN, makanTokenKurungBulatTutup()} // constructor Titik
        )                                                                                                  // constructor make_unique
    );                                                                                                     // function tambahTitikTurunan
  } // else
...
std::unique_ptr<nusantara::Titik> nusantara::PenguraiSintaks::uraiEkspresi()
{
  ...
  if (this->cekTipeToken(TipeToken::nilaiBilangan))
  {
    titik.tambahTitikTurunan(std::make_unique<Titik>(Titik{
        TipeTitik::TOKEN,
        this->makanToken(TipeToken::nilaiBilangan,
            "Bukanlah sebuah bilangan.") // function makanToken
    })                                   // constructor make_unique
    );                                   // function tambahTitikTurunan
  }
  else if (this->cekTipeToken(TipeToken::nilaiTeks))
  {
    titik.tambahTitikTurunan(std::make_unique<Titik>(Titik{
        TipeTitik::TOKEN,
        this->makanToken(TipeToken::nilaiTeks,
            "Bukanlah sebuah teks.") // function makanToken
    })                               // constructor make_unique
    );                               // function tambahTitikTurunan
  }
  else
  {
    throw DataPengecualianSintaks{.lokasiBerkas = this->tokenSaatIni().lokasiBerkas, .lokasiToken = this->tokenSaatIni().lokasi, .konten = this->tokenSaatIni().konten, .pesan = __NK__LABEL_KELUARAN_PS__ "Ekspresi tidak benar."}; // throw
  } // else
