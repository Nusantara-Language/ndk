...
int nusantara::abp(const int& argc, const char*& argv)
{
  ...
  argparse::ArgumentParser program("nusantara", VERSI, argparse::default_arguments::none, true, std::cout);
  ...
  program.add_argument("-v", "--versi")
      ...
      .nargs(0)
      .action(
          [&program](const std::string& nilai)
          {
...
#endif
            return std::pair<std::string, std::string>({lokasiBerkas, nusantara::bacaBerkasDalamBentukString(lokasiBerkas)});
          });

  auto& argumenProsesBerkas = program.add_mutually_exclusive_group();

  argumenProsesBerkas.add_argument(__NK__ARGUMEN_PROSES_BERKAS_TS__).help("Untuk melakukan tokenisasi berkas nusantara.").nargs(2).default_value(false).implicit_value(true).flag();

  argumenProsesBerkas.add_argument(__NK__ARGUMEN_PROSES_BERKAS_AS__).help("Untuk melakukan analisis semantik berkas nusantara.").nargs(2).default_value(false).implicit_value(true).flag();

  argumenProsesBerkas.add_argument(__NK__ARGUMEN_PROSES_BERKAS_PS__).help("Untuk mengurai berkas nusantara.").nargs(2).default_value(false).implicit_value(true).flag();

  argumenProsesBerkas.add_argument(__NK__ARGUMEN_PROSES_BERKAS_PSA__).help("Untuk mengurai berkas nusantara.").nargs(2).default_value(false).implicit_value(true).flag();

  argumenProsesBerkas.add_argument(__NK__ARGUMEN_PROSES_BERKAS_P__).help("Untuk menafsirkan berkas nusantara.").nargs(2).default_value(false).implicit_value(true).flag();
  ...
  auto berkas = program.get<std::pair<std::string, std::string>>("<berkas"
#ifdef __NK__EKSTENSI_BERKAS_WAJIB__
                                                                 "." __NK__EKSTENSI_BERKAS__
#endif
                                                                 ">"); // fungsi get
  ...
  if (berkas.first.empty() && berkas.second.empty())
  {
    if (tsArg || psArg || psaArg || asArg || pArg)
      cetakDBB("Berkas belum ditentukan.");
    nusantara::cetak(program);
    exit(0);
  } // if
