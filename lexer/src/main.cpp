...
auto main(int argc, const char *argv[]) -> int {
  ...
  for (auto &token : tokens) {
    std::cout << std::setw(filenameWidth) << std::left << token.fileName << ":"
              << std::setw(lineWidth) << std::left << token.location.row << ":"
              << std::setw(indexWidth) << std::left << token.location.column
              << " " << std::setw(typeWidth) << std::left
              << lexer::tokenTypeToString(token.type) << " => "
              << std::setw(contentWidth) << std::left << token.content << "\n";
  }
